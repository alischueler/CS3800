...
public class CFGMaker implements CFG {
    ...
    public PDA toPDA() {
        ...
        State start = makePDAStates(states, accept);
        List<PDATransition> pdatransitions = makePDATransitions(states, start, accept);
        return new PDAMaker(states, start, accept, pdatransitions);
    }

    public State makePDAStates(List<State> states, List<State> accept) {
        State s1 = new State(Integer.toString(0), "qstart");
        State s2 = new State(Integer.toString(1), "qloop");
        State s3 = new State(Integer.toString(2), "qaccept");
        states.add(s1);
        states.add(s2);
        states.add(s3);
        accept.add(s3);
    ...
    public List<PDATransition> makePDATransitions(List<State> states, State start, List<State> accept) {
        List<PDATransition> list = new ArrayList<>();
        String p = variables.get(0).toString()+"$";
        PDATransition one = new PDATransition(start.getId(), states.get(1).getId(), "empty", "empty", p);
        list.add(one);
        for (Map.Entry<Variable, LinkedHashSet<String>> m : rules.entrySet()) {
            Variable v = m.getKey();
            for (String s : m.getValue()) {
                ...
                String push;
                    ...
                    pop = v.toString();
                    push = s;
                PDATransition trans = new PDATransition(states.get(1).getId(), states.get(1).getId(), read, pop, push);
                list.add(trans);
            }
        }
        for (Terminal t : terminals) {
            PDATransition trans = new PDATransition(states.get(1).getId(), states.get(1).getId(), t.toString(), t.toString(), "empty");
            list.add(trans);
        }
        PDATransition last = new PDATransition(states.get(1).getId(), accept.get(0).getId(), "empty", "$", "empty");
        list.add(last);
        return list;
    }
