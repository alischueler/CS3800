...
public class DFAMaker implements DFA {
    ...
    public void setUpUnionTransitions(List<String> los) {
        ...
        for (int i = 0; i < los.size(); i++) {
            String[] broken = los.get(i).split(" ");
            if (delta.containsKey(broken[0] + " " + broken[1])) {
                LinkedHashMap<String, String> inner = delta.get(broken[0] + " " + broken[1]);
                inner.put(broken[4], broken[2] + " " + broken[3]);
                delta.replace(broken[0] + " " + broken[1], inner);
            } else {
                LinkedHashMap<String, String> inner = new LinkedHashMap<>();
                inner.put(broken[4], broken[2] + " " + broken[3]);
                delta.put(broken[0] + " " + broken[1], inner);
            }
        }
    }
